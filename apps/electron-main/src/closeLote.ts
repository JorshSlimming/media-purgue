...
export async function closeLote(lotePath: string) {
  ...
  for (const [idx, f] of conservados.entries()) {
    ...
    try {
      await withRetries(async () => { await copyFile(f.ruta_original, dest) })
      ...
      await writeLogJSON(path.join(mpRoot, 'Logs'), `lote_${lote.lote_id}.json`, { lote_id: lote.lote_id, error: err?.message || 'copy_failed', entries: logEntries, fecha: new Date().toISOString() })
      throw err
    }
  }
  ...
  for (const [idx, f] of conservados.entries()) {
    ...
    try {
      ...
      await withRetries(async () => { await moveFile(src, destFinal) })
      ...
      await writeLogJSON(path.join(mpRoot, 'Logs'), `lote_${lote.lote_id}.json`, { lote_id: lote.lote_id, error: err.message, entries: logEntries, fecha: new Date().toISOString() })
  ...
  for (const [idx, f] of eliminados.entries()) {
    try {
      await unlinkFile(f.ruta_original)
