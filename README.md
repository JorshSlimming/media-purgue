...
- **Segura** (sin riesgo de borrar lo correcto)
...
El usuario selecciona una **carpeta de origen** (por ejemplo, "Fotos" o "Videos") que contiene todos los archivos a revisar. Dentro de esa carpeta se creará una carpeta oculta llamada `.media-purgue` que contendrá toda la estructura temporal del proyecto. Al finalizar, la biblioteca con los archivos conservados se moverá al mismo nivel que la carpeta de origen (o a la ubicación configurada), y la carpeta temporal se eliminará.
...
Carpeta_de_origen/
 ...
 └── .media-purgue/                          # Carpeta oculta temporal
      ...
      │     ├── Imagenes_Lote_0001/
      │     │     └── lote_0001.json            # Metadatos del lote
      │     ├── Imagenes_Lote_0002/
      │     │     └── lote_0002.json
      │     └── Videos_Lote_0001/
      │           └── lote_0001.json
...
| Criterio de creación de lotes | Orden de los archivos dentro de los lotes | `fecha creación` |
...
El usuario abre un lote desde la interfaz. La aplicación lee el JSON correspondiente y muestra un archivo por vez, cargando la vista previa directamente desde `ruta_original`. Opciones:
...
1. **Preparación**: Se lee el JSON del lote y se generan dos listas:
    - Archivos a conservar (estado = "conservar")
    - Archivos a eliminar (estado = "eliminar")
2. **Fase de copia/movimiento a staging**:
...
3. **Fase de confirmación en Biblioteca Final**:
